...
type ASIOCapturer struct {
	...
	stopCh         chan struct{}
	doneCh         chan struct{}
	callbackOnce   sync.Once    // logs first callback; resets with each new instance
	callbackFired  atomic.Bool  // set true on first callback; used by the watchdog
}
...
func (c *ASIOCapturer) Start(ctx context.Context) error {
	...
	go func() {
		asioGlobalMu.Lock() // one ASIO driver open at a time
...
// OpenASIOControlPanel öffnet das Treiber-Panel. Da der Client eine Tray-App
// mit eigenem Fenster ist, hat der Prozess Foreground-Status → Panel öffnet vorne.
