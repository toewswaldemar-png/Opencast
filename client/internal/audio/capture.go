...
// WASAPI loopback stream flag (not exported by go-wca).
...
func (c *WasapiCapturer) readAvailablePackets(cc *wca.IAudioCaptureClient) {
	for {
		...
		case c.srcBPS == 2:
			pcm = raw
		default:
			pcm = intToInt16LE(raw, c.srcBPS, c.actual.Channels)
		}
...
// intToInt16LE converts interleaved integer PCM (24- or 32-bit LE) to 16-bit LE.
func intToInt16LE(src []byte, srcBPS uint32, channels uint16) []byte {
	n := int(srcBPS)
	if n == 0 || len(src) == 0 {
		return nil
	}
	...
	for i := range samples {
		...
		var v int32
		switch n {
		case 3: // 24-bit signed LE
			v = int32(src[off]) | int32(src[off+1])<<8 | int32(int8(src[off+2]))<<16
			v <<= 8 // scale to 32-bit range
		case 4: // 32-bit signed LE
