...
// NewCapturer returns an ASIOCapturer for "asio:<clsid>" device IDs
// and a WasapiCapturer for all other device IDs.
...
/* ------------------------------------------------------------------ */
/* WasapiCapturer (shared with capture.go logic, ASIO build variant)  */
/* ------------------------------------------------------------------ */
...
func (c *WasapiCapturer) initCapture() (*wca.IMMDeviceEnumerator, *wca.IAudioClient, *wca.IAudioCaptureClient, error) {
	...
	c.isFloat = wasapiIsFloatFormat(mixFmt)
...
var wasapiKsdataformatSubtypeIEEEFloat = [16]byte{
	0x03, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00,
	0x80, 0x00, 0x00, 0xAA, 0x00, 0x38, 0x9B, 0x71,
}

func wasapiIsFloatFormat(f *wca.WAVEFORMATEX) bool {
	...
	if f.WFormatTag == waveFormatExtensible {
		...
		return *subFmt == wasapiKsdataformatSubtypeIEEEFloat
	}
	return false
}
...
func (c *WasapiCapturer) readAvailablePackets(cc *wca.IAudioCaptureClient) {
	for {
		...
		var pcm []byte
		if c.isFloat {
			pcm = wasapiFloat32ToInt16LE(raw)
		} else {
			pcm = raw
		}

		c.sendWasapiLevels(pcm)
...
func wasapiFloat32ToInt16LE(src []byte) []byte {
	n := len(src) / 4
	out := make([]byte, n*2)
	for i := range n {
...
func (c *WasapiCapturer) sendWasapiLevels(pcm []byte) {
	if len(pcm) < 2 {
		return
	}
